...
def find_highconf_sex_chr(input_file,reads_depth):
    highconf_locus = []
    highconf_parents_locus = []
    highconf_dict = defaultdict(list)
    highconf_parents_dict = defaultdict(list)
    ...
    for line in f:
        ...
        if line[:2] == "#C":
            ...
            try:
                ...
                # second
                if (samples[0].GT_hp1=="0/0" or samples[0].GT_hp1=="1/1") and (samples[0].GT_hp2=="0/0" or samples[0].GT_hp2=="1/1") and (samples[1].GT_hp1=="0/0" or samples[1].GT_hp1=="1/1") and (samples[1].GT_hp2=="0/0" or samples[1].GT_hp2=="1/1"):
                    for ii in range(2):
                        if samples[ii].GT_hp1=="0/0":
                            GT_hp1_count[ii] = samples[ii].RO_hp1
                        elif samples[ii].GT_hp1=="1/1":
                    ...
                    if GT_hp1_count[0] >=reads_depth and GT_hp2_count[0]>=reads_depth and GT_hp1_count[1] >=reads_depth and GT_hp2_count[1] >=reads_depth:
                        highconf_locus.append(data[1])
                        highconf_dict[int(data[1])] = [samples[0].RO_hp1,samples[0].AO_hp1,samples[0].RO_hp2,samples[0].AO_hp2,samples[1].RO_hp1,samples[1].AO_hp1,samples[1].RO_hp2,samples[1].AO_hp2] 
                        highconf_dict2[int(data[1])] = 1 
...
def find_denovo_mut(vcf_file,threehps_vcf,reads_depth,fw,child_num,parent1_num,parent2_num,denovo_file,chr_num,fw_highconf):
    if chr_num == 23 or chr_num == 24:
        highconf_locus,highconf_dict,highconf_dict2,highconf_parents_locus,highconf_parents_dict,highconf_parents_dict2 = find_highconf_sex_chr(threehps_vcf,reads_depth)
    else:
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        ...
        fw_highconf = open("HighConf.txt","w")
        ...
        for chr_num in range(chr_start,chr_end+1): 
            ...
            try:
                count_lowconf = find_denovo_mut(phased_vcf,threehps_vcf,reads_depth,fw, child_num, parent1_num, parent2_num,denovo_raw_file,chr_num,fw_highconf)
            except:
